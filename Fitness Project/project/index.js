...
// An object giving Passport the data Google
...
async function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    let name = await dbo.checkDB(profile.id)
    let userid = profile.id;
 

    if (name === undefined){
      await dbo.post_profile(profile);
      console.log("insert into profile db")
      }else{
...
passport.deserializeUser(async (userid, done) => {
    ...
    let givenname = await dbo.get_name(userid);
    if(givenname!= undefined){
      // name =  JSON.stringify(givenname.givenname);
      userData = givenname;
    };    
    // let name = await dbo.get_name(request.user.userid);
    // here is a good place to look up user data in database using
    // dbuserID. Put whatever you want into an object. It ends up
...
app.post('/store', isAuthenticated, async function(request, response, next) {
  ...
  let totalData = Object.assign(request.body, request.user)
  
  let activity = act.Activity(totalData)
...
app.get('/reminder', isAuthenticated,  async function(request, response, next) {
  ...
  if (result != null){
    ...
    response.send(act.ReminderActivity(result));
  } else {
...
app.get('/name', isAuthenticated,  async function(request, response, next) {
  ...
  // response.send(name.givenname);
  response.send(request.user.givenname);
});
...
app.get('/week', isAuthenticated, async function(request, response, next) {
  ...
  console.log("result of week plot", result);
